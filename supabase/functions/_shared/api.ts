// Shared API envelope and credential validation. Raw keys are never logged.
...
export type Principal =
  | { kind: "api_key"; id: string; organizationId: string | null; scopes: string[] }
  | { kind: "user"; id: string };
...
function looksLikePlatformApiKey(raw: string): boolean {
  return raw.startsWith("uc_") || raw.startsWith("scene_");
}

export async function validateApiKey(req: Request): Promise<Principal | null> {
  const auth = req.headers.get("authorization") ?? "";
  const raw = auth.startsWith("Bearer ") ? auth.slice(7).trim() : "";
  if (!looksLikePlatformApiKey(raw)) return null;

  const { data, error } = await admin()
    .schema("system")
    .rpc("validate_api_key", { p_raw: raw });
